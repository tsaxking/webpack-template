<script lang="ts">
    ...
    export let color: 'info' | 'success' | 'danger' | 'warning' | 'secondary' | 'light' = 'info';
...
<div class={show ? 'show ' + classes : classes} role="alert" aria-live="assertive" aria-atomic="true" on:mouseover={update} on:focus={update}>
    <div class="toast-header bg-dark border-0 text-{color}">
        ...
        <small>{time}</small>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="toast" aria-label="Close" on:click={() => dispatch('hide.bs.toast')}></button>
    </div>
    <div class="toast-body bg-{color} {bodyTextColor ? 'text-' + bodyTextColor : ''}">
        {message}
        <slot></slot>
    </div>
</div>